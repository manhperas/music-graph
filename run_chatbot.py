...
def run_gradio(args):
    ...
    if args.load_model:
        try:
            ...
            chatbot = Qwen3MusicChatbot(model_name='Qwen/Qwen3-0.6B', lora_path=args.lora_path, quantization='4bit' if not args.no_quantize else None)
    ...
    launch_gradio(chatbot=chatbot, graph_retriever=retriever, port=args.port, share=args.share)
...
def run_api(args):
    ...
    uvicorn.run('src.api.server:app', host='0.0.0.0', port=args.api_port, reload=args.reload, log_level='info')

def run_both(args):
    import threading
    import time
    api_thread = threading.Thread(target=run_api, args=(args,), daemon=True)
    api_thread.start()
    time.sleep(2)
    run_gradio(args)

def main():
    parser = argparse.ArgumentParser(description='Run Music Knowledge Graph Chatbot', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n    # Demo mode (no model required)\n    python run_chatbot.py --ui gradio\n    \n    # With model loaded\n    python run_chatbot.py --ui gradio --load-model\n    \n    # With custom LoRA path\n    python run_chatbot.py --ui gradio --load-model --lora-path /path/to/lora\n    \n    # FastAPI server\n    python run_chatbot.py --ui api\n    \n    # With Neo4j graph\n    python run_chatbot.py --ui gradio --load-model --use-neo4j\n        ')
    parser.add_argument('--ui', choices=['gradio', 'api', 'both'], default='gradio', help='Which UI to run (default: gradio)')
    parser.add_argument('--load-model', action='store_true', help='Load the actual Qwen model (requires GPU)')
    parser.add_argument('--lora-path', type=str, default=None, help='Path to LoRA adapter (optional)')
    parser.add_argument('--no-quantize', action='store_true', help='Disable 4-bit quantization')
    parser.add_argument('--use-neo4j', action='store_true', help='Connect to Neo4j for graph context')
