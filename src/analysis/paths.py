"""Shortest path analysis module for music network"""
...
class PathAnalyzer:
    """Analyze shortest paths between artists in the music network"""
    ...
    def find_shortest_path(self, graph: nx.Graph, artist1_name: str, artist2_name: str) -> Optional[Dict]:
        ...
        Args:
            ...
            artist1_name: Name of first artist
            artist2_name: Name of second artist
    ...
    def find_all_shortest_paths(self, graph: nx.Graph, artist1_name: str, artist2_name: str) -> List[Dict]:
        """
        Find all shortest paths between two artists
    ...
    def get_artist_shortest_paths_summary(self, graph: nx.Graph, limit: int = 20) -> Dict:
        """
        Get summary of shortest paths for a sample of artists
        
        Args:
            graph: NetworkX graph
            limit: Number of artist pairs to analyze
        ...
        try:
            ...
            for a1, a2 in pairs[:limit]:
                ...
                if result and result.get('path_exists'):
                    ...
                    if len(path_examples) < 5:  # Keep first 5 examples
                        path_examples.append(result)
            
            if not path_lengths:
                return {
                    'num_pairs_analyzed': len(pairs),
                    'num_paths_found': 0,
            ...
            summary = {
                ...
                'avg_path_length': float(sum(path_lengths) / len(path_lengths)),
                'min_path_length': int(min(path_lengths)),
                'max_path_length': int(max(path_lengths)),
                'median_path_length': float(sorted(path_lengths)[len(path_lengths) // 2]),
                'examples': path_examples[:5]
            }
    ...
    def compute_average_path_length(self, graph: nx.Graph) -> Optional[float]:
        """
        Compute average shortest path length for the network
        
        Args:
            graph: NetworkX graph
            
        Returns:
            Average path length or None if not computable
        """
        try:
            undirected_graph = graph.to_undirected()
    ...
    def compute_diameter_and_radius(self, graph: nx.Graph) -> Dict:
        """
        Compute diameter and radius of the network
        
        Args:
            graph: NetworkX graph
            
        Returns:
            Dictionary with diameter and radius
        """
        try:
            undirected_graph = graph.to_undirected()
...
def analyze_paths(graph_path: str = "data/processed/network.graphml",
                  output_path: str = "data/processed/path_analysis.json") -> Dict:
    """Main function to analyze shortest paths"""
