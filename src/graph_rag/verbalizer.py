"""
Triple Verbalizer for GraphRAG.

This module converts graph triples into natural language descriptions
for use in LLM context construction.
"""
...
class TripleVerbalizer:
    """Converts graph triples to natural language."""

    def __init__(self):
        self.templates = {
            'COLLABORATES_WITH': [
                "{subject} collaborated with {object}",
                "{subject} worked together with {object}",
                "{subject} and {object} have collaborated",
                "{subject} has worked with {object}"
            ],
            'PERFORMS_ON': [
                "{subject} performs on the album {object}",
                "{subject} released {object}",
                "{subject}'s album {object}",
                "{object} is an album by {subject}"
            ],
            'HAS_GENRE': [
                "{subject} has the genre {object}",
                "{subject} is {object} music",
                "{subject} belongs to {object} genre",
                "{subject} plays {object}"
            ],
            'WON_AWARD': [
                "{subject} won {object}",
                "{subject} received the award {object}",
                "{object} was awarded to {subject}",
                "{subject} is a {object} winner"
            ],
            'MEMBER_OF': [
                "{subject} is a member of {object}",
                "{subject} belongs to the band {object}",
                "{subject} plays in {object}",
                "{object} includes {subject} as a member"
            ],
            'RELEASED': [
                "{subject} released {object}",
                "{object} was released by {subject}",
                "{subject} put out {object}",
                "{object} is a release by {subject}"
            ],
            'BELONGS_TO': [
        ...
        self.default_templates = [
            "{subject} is connected to {object} through {relation}",
            "{subject} has a relationship with {object}: {relation}",
            "{subject} and {object} are linked by {relation}"
        ]
    ...
    def verbalize_triple(self, triple: Tuple[str, str, str]) -> str:
        ...
        # Get templates for this relation type
        templates = self.templates.get(relation, self.default_templates)

        # Use first template (could be randomized for variety)
        template = templates[0]

        # Format the template
        verbalization = template.format(
            subject=self._format_entity_name(subject),
            object=self._format_entity_name(obj),
            relation=self._format_relation_name(relation)
        )
    ...
    def build_context_from_triples(self, triples: List[Tuple[str, str, str]],
                                 query: str = "") -> str:
        """
        Build coherent context paragraph from triples.
        ...
        if len(verbalizations) == 1:
            ...
            # Join most with commas, last with "and"
    ...
    def _format_entity_name(self, name: str) -> str:
        """
        Format entity name for natural language.
        ...
        """
        # Add quotes around names that contain spaces or special characters
    ...
    def get_relation_summary(self, triples: List[Tuple[str, str, str]]) -> Dict[str, Any]:
        """
        Generate summary of relations in triples.
